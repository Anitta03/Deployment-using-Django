...
class PictureList(APIView):
    ...
    def post(self,request, format = None):
        try:
            ...
            # mew = tomato.objects.first().image.url
            # print(mew)
            
            # new = list(serializer.data.values())
            # im = new[0]
            # base_path = Path.cwd()
            # file_path = os.path.join(base_path, mew)
            # print(file_path)
               
            # imge=Image.open(im)
            file_path = list(serializer.data.values())[0]
            # one = urllib.request.urlretrieve( file_path,) 
            # print(file_path)
            ...
            return Response('Your Prediction is {}'.format(preds), status=status.HTTP_201_CREATED)
